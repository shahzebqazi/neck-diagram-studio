...
const router = Router();
const ALLOWED_TYPES = new Set<string>(["scale", "mode", "shape", "position", "key"]);

router.get("/", async (req, res) => {
  const query = typeof req.query.query === "string" ? req.query.query : "";
  const rawType = typeof req.query.type === "string" ? req.query.type : undefined;
  const type: LibraryType | undefined =
    rawType && ALLOWED_TYPES.has(rawType) ? (rawType as LibraryType) : undefined;
  if (rawType && !type) {
